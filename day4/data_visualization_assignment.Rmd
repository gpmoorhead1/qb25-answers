...
ggplot(dt, aes(x=Prostate)) +
  geom_histogram(breaks = seq(0, 100, 2)) +
  theme_classic()
# answer here
# The distribution is skewed towards 0, meaning that the vast majority of genes are expressed at low TPMs in the liver. It seems that this is not specific to the liver.
...
dt %>% mutate(is_mitochondrial = str_starts(dt$Description, 'MT')) %>% 
  ggplot(aes(x=Liver+1, y=`Heart - Left Ventricle`+1, color=is_mitochondrial)) +
    geom_point(alpha=0.2, size = 1) +
    scale_x_log10() + 
    scale_y_log10() +
    theme_classic() +
...
```{r}
dt_long <- dt %>% mutate(is_mitochondrial = str_starts(dt$Description, 'MT')) %>%
